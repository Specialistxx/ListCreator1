...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (interaction.isButton()) {
      ...
      if (!farm) return interaction.reply({ content: '⚠️ This farm is no longer active.', ephemeral: true });

      const farmMessage = await interaction.channel.messages.fetch(farmMessageId).catch(() => null);
      if (!farmMessage) return interaction.reply({ content: '⚠️ Cannot find farm message.', ephemeral: true });
      ...
      // 📣 PING EACH PLAYER INDIVIDUALLY (stable)
      if (action === 'ping') {
        if (!isHostOrAdmin)
        ...
        try {
          ...
          return interaction.reply({ content: '✅ All players have been pinged individually.', ephemeral: true });
        } catch {
          return interaction.reply({ content: '⚠️ Failed to send pings.', ephemeral: true });
        }
      }

      // FINALIZE / UNFINALIZE TOGGLE (fully fixed)
      if (action === 'finalize') {
        if (!isHostOrAdmin)
          return interaction.reply({ content: '🚫 Admins/Host only.', ephemeral: true });
        ...
        const statusMsg = farm.finalized
